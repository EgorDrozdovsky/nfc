...
class StoreController extends Controller
{
    ...
    {
        $themes = Theme::where('theme_description', 'WhatsApp Store')->where('status', 1)->first();
    ...
    public function saveStore(Request $request)
    {
        ...
        if ($validator->fails()) {
            alert()->error('Some fields missing or banner/logo size is large.');
        ...
        if ($current_card == 0) {
            ...
            if ($cards < $plan_details['no_of_vcards']) {
                try {
                    ...
                    alert()->success('New WhatsApp Store Created Successfully!');
                ...
                alert()->error('Maximum store creation limit is exceeded, Please upgrade your plan to add more store(s).');
                return redirect()->route('user.create.store');
            }
        } else {
            alert()->error('Sorry, personalized link was already registered.');
    ...
    public function saveProducts(Request $request, $id)
    {
        ...
        if ($business_card == null) {
            ...
            if($request->badge != null){
             ...
             alert()->error('You must add atleast one product.');
                return redirect()->route('user.products', $id);
            }
