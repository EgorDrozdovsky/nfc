    ...
    <div class="container-xl">
        ...
        <h1 class="navbar-brand navbar-brand-autodark d-none-navbar-horizontal pe-0 pe-md-3">
            <a href="{{ route('user.dashboard') }}">
                <img src="{{ url('/') }}{{ $settings->site_logo }}" alt="{{ $settings->site_name }}" width="110"
                    height="32" class="navbar-brand-image">
            </a>
        </h1>
            ...
            <div class="nav-item dropdown">
                <a href="#" class="nav-link d-flex lh-1 text-reset p-0" data-bs-toggle="dropdown"
                    ...
                    <span class="avatar avatar-sm"
                        style="background-image: url({{ Auth::user()->profile_image == null ? '/profile.png' : auth::user()->profile_image }})"></span>
                    <div class="d-none d-xl-block ps-2">
                        <div>{{ Auth::user()->name }}</div>
                        <div class="mt-1 small text-muted">{{ __('User') }}</div>
                    </div>
                </a>
