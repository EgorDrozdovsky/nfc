...
<body class="antialiased bg-body text-body font-body"
    ...
    <div class="">
        ...
        <section class="py-8">
            <div class="container px-4 mx-auto">
                <div class="h-60 bg-white rounded overflow-hidden">
                    <img class="rounded top-0 right-0 object-cover"
                        ...
                        <div class="p-4 bg-white shadow rounded">
                            <div class="relative h-40 w-full mb-4">
                                <img class="w-full h-full object-cover rounded"
                                    id="{{ $product->product_id }}_product_image" src="{{ asset($product->product_image) }}"
                                    alt="{{ $product->product_name }}">
                                <span
                                    class="absolute top-0 right-0 py-1 px-2 mt-2 mr-1 bg-{{ $product->product_status == "instock" ? "green" : "red" }}-500 rounded text-xs text-white">{{ $product->product_status == "instock" ? "In Stock" : "Out of Stock" }}</span>
                                <span
                                    class="absolute top-0 right-20 py-1 px-2 mt-2 ml-2 bg-red-500 rounded text-xs text-white">{{ $product->badge }}</span>
                            </div>
                            <div class="flex mb-6 justify-between items-center">
                                <div>
                                    <h3 id="{{ $product->product_id }}_product_name" class="text-md font-medium">
                            ...
                            <div class="flex mb-1 justify-between items-center">
                                <h4 class="text-xl font-bold"><span
                                    ...
                                    <span class="text-sm line-through text-red-500 font-bold">
                                        {{ $currency }}{{ $product->regular_price }}</span>
                                    @endif
                                </h4>
                                @if ($product->product_status == "instock")
                                <a onclick="addToCart('{{ $product->product_id }}')"
                                    class="py-1 px-3 bg-{{ $business_card_details->theme_color }}-500 hover:bg-{{ $business_card_details->theme_color }}-600 rounded text-md text-white transition duration-200">{{ __('Add to cart') }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <ul class="grid grid-flow-col lg:grid-cols-12 grid-cols-6 grid-rows-1">
                    ...
                    <a target="_blank" href="{{ $shareComponent['facebook'] }}">
                        <li class="flex cursor-pointer items-center">
                            <div
                                class="flex justify-center items-center content-center bg-gradient-to-br from-indigo-300 to-indigo-600 shadow-md hover:shadow-lg h-12 w-12 rounded-full fill-current text-white">
                    ...
                    <a target="_blank" href="{{ $shareComponent['twitter'] }}">
                        <li class="flex cursor-pointer items-center">
                            <div
                                class="flex justify-center items-center content-center bg-gradient-to-br from-blue-300 to-blue-600 shadow-md hover:shadow-lg h-12 w-12 rounded-full fill-current text-white">
                    ...
                    <a target="_blank" href="{{ $shareComponent['whatsapp'] }}">
                        <li class="flex cursor-pointer items-center">
                            <div
                                class="flex justify-center items-center content-center bg-gradient-to-br from-green-300 to-green-600 shadow-md hover:shadow-lg h-12 w-12 rounded-full fill-current text-white">
        ...
        <div class="my-4 mt-12">
            <p class="mb-2 text-center text-base text-gray-500">{{ __('Copyright') }} &copy; <span id="year"></span>.
            </p>
        </div>
        @else
        <div class="my-4 mt-12">
    ...
    <script>
        ...
        function updateList() {
            ...
            for (let j = 0; j < cart.length; j++) {
                ...
                cart_items += '<div class="p-4 bg-white rounded"><div class="relative h-20 w-full mb-4"> <img class="w-full h-full object-cover rounded" src="' + cart[j].product_image + '"></div><div class="flex mb-6 justify-between items-center"><div><h3 class="text-sm font-medium">' + cart[j].product_name + '</h3> <span class="text-xs text-gray-500">' + cart[j].subtitle + '</span></div></div><div class="flex mb-2 justify-between items-center"><h4 class="text-xl font-bold">' + currency + ' ' + total_price + '</h4> <a onclick="reduceQty(' + j + ')" class="py-2 px-3 bg-red-500 hover:bg-red-600 rounded-full text-xs text-white transition duration-200">-</a><h4 class="text-sm font-medium">' + cart[j].qty + '</h4> <a onclick="addQty(' + j + ')" class="py-2 px-3 bg-red-500 hover:bg-red-600 rounded-full text-xs text-white transition duration-200">+</a> <a class="py-2 px-3 bg-red-500 hover:bg-red-600 rounded-full text-xs text-white transition duration-200" onclick="removeFromCart(' + j + ')">X</a></div></div>';
            }
            cart_items += '<br> <h3 class="pl-4 pt-4 pr-4 font-bold">{{ __("Grand total:") }} ' + currency + ' ' + grandTotal + '</h3>';
